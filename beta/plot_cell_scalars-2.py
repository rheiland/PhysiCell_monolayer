...
class Vis():
    ...
    def plot_cell_scalar(self, frame):
        ...
        if cell_scalar_mcds_name == "beta_or_gamma": 
            ...
            if (len(self.discrete_variable) == 1): 
                ...
                try:
                    ...
                    # dark blue, light blue, yellow/gold, red
                    # cbar_name = from_list(None, [[0.0, 0, 1.0],[0,0.5,1],[1,1,0],[250/255.,0,0]], len(self.discrete_variable))
                    cbar_name = from_list(None, [[35/255.,102/255.,255/255.],[157/255.,207/255.,230/255.],[250/255.,159/255.,80/255.],[204/255.,0,24/255.]], len(self.discrete_variable))
                    # cbar_name = from_list(None, [0,0,1],[0,0.5,1],[1,1,0],[250/255.,159/255.,80/255.], len(self.discrete_variable))
                    # cbar_name = from_list(None, [[0,0,1],[0,0.5,1],[1,1,0],[1,0,0]], len(self.discrete_variable))
        ...
        hrs = hrs-days*24 + (mins-hrs*60)/60.
        self.title_str = f" {self.output_dir} ({days} days, {hrs:.1f} hrs)"
