...
p = np.concatenate(([0], 1/k * eqns[-1, :])) + np.concatenate((eqns[-1, :], [0])) - np.concatenate((eqns[-2, :], [0]))

# Find roots
L = np.roots(p[::-1])  # numpy uses opposite convention for polynomial coefficients

# Build matrices
A = np.zeros((num_cells, num_cells))
L_pow = np.zeros((num_cells, num_cells))
