...
dt_mechanics = 1.0
# dt_mechanics = 2.0
max_relax_steps = int(1/dt_mechanics)  # not used
...
class Agent:
    ...
    growth_rate: float = 0.1772   # microns^2(?)/min  0.05
...
class Simulation:
    ...
    def step(self) -> int:
        ...
        for relax_steps in range(1):   # shouldn't need relaxation steps now
...
if __name__ == "__main__":
    ...
    sim = Simulation(
        ...
        growth_rate= 0.1772,    # 0.889 original;  5x slower=0.1778  (micron^2 / min)
    ...
    sim.precompute(steps=500000, max_agents=MAX_AGENTS)  # rwh: increase # steps for smaller dt_mechanics 
